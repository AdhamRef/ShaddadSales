  ...
  return (
    <nav className="bg-card border-b border-border px-8 py-4 flex justify-between items-center shadow-sm">
      <div className="flex items-center gap-3">
        <div className="w-10 h-10 bg-gradient-to-br from-accent to-blue-600 rounded-xl flex items-center justify-center text-card font-semibold text-lg">
          {session?.user?.name?.charAt(0).toUpperCase()}
        </div>
        <div>
          <span className="font-bold text-lg text-foreground">أداء المبيعات</span>
          <div className="text-xs text-muted-foreground">لوحة الوكيل</div>
        </div>
      </div>
      <div className="flex items-center gap-6">
        <div className="text-right">
          <div className="text-sm font-semibold text-foreground">{session?.user?.name}</div>
          <div className="text-xs text-accent font-medium">وكيل</div>
        </div>
